...
function renderAdmin(){if(!isAdmin()){state.view='home';render();return}setHeader('ADMIN CONTROL','Manage Household');const pc=realChoresEnabled()?[]:state.chores.filter(c=>c.status==='Pending'),pr=state.redemptions.filter(r=>r.status==='Pending'&&!r.backend);
 const householdPanel=`<div class="card admin-section"><div class="section-title"><h3>Account</h3></div><div class="row"><div><h4>Signed in as ${esc(state.currentUser)}</h4><p class="muted">Account controls</p></div><button class="ghost" type="button" data-auth-action="signout">↪ Sign Out</button></div></div>`;
const seasonPanel=`
...
if(btn.dataset.view){
 ...
 if(window.FQAuth?.realSession&&['rewards','cosmetics','home','profiles'].includes(target)){
   Promise.all([window.FQAuth.refreshIdentity?.(),target==='rewards'?loadRealRewards():Promise.resolve(),target==='cosmetics'?loadRealCosmetics():Promise.resolve()]).then(()=>render()).catch(err=>{console.error(err);render()});
 }else render();
 return
}
