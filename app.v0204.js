...
async function realRequestReward(r,details){
 const {data,error}=await window.FQAuth.client.rpc('request_reward_v2',{p_reward_id:r.id,p_details:details||{}});
 ...
 const detailsOut=resultData.details||{};
 const lotteryResult=detailsOut.lotteryResult||'';
 if(redemptionId&&!state.redemptions.some(x=>String(x.id)===String(redemptionId))){
   state.redemptions.unshift({
     id:redemptionId,user:state.currentUser,userId:realProfile()?.user_id||me?.id,
     rewardId:r.id,title:r.title,templateKey:r.templateKey,cost:r.cost,
     status:({pending:'Pending',approved:'Approved',denied:'Denied',resolved:'Resolved'})[status]||status,
     details:detailsOut,month:monthKey(),requestedAt:new Date().toISOString(),backend:true
   });
 }
 render();
 if(lotteryResult){showLotteryResult(lotteryResult);toast(`🎟️ ${lotteryResult}`)}
 if(r.kind==='multiplier')toast(`⚡ ${r.templateKey==='triple_xp_day'?'3×':'2×'} XP ACTIVE UNTIL MIDNIGHT`);
 // Background reconciliation only. The authoritative response above already updated the screen.
 Promise.allSettled([loadRealRewards(),loadRealNotifications(),loadRealXpPowerups()]).then(()=>{});
...
function openReward(id){
 ...
 if(r.kind==='lottery'&&r.backend){
   ...
   realRequestReward(r,{}).then(res=>{if(!res?.ok)console.warn('Lottery request rejected',res)}).catch(err=>{console.error(err);toast('Lottery request failed.');});
   return
 }
