...
async function getPushRegistration(){if(!pushSupported())return null;return navigator.serviceWorker.register('./sw.js')}
...
let quickNavTouchAt=0;document.addEventListener('pointerup',e=>{const card=e.target.closest?.('[data-view-jump].quick-stat');if(!card||e.pointerType==='mouse')return;e.preventDefault();quickNavTouchAt=Date.now();state.view=card.dataset.viewJump;render()},{passive:false});document.addEventListener('touchend',e=>{const card=e.target.closest?.('[data-view-jump].quick-stat');if(!card||Date.now()-quickNavTouchAt<500)return;e.preventDefault();quickNavTouchAt=Date.now();state.view=card.dataset.viewJump;render()},{passive:false});document.addEventListener('click',e=>{if(Date.now()-quickNavTouchAt<500&&e.target.closest?.('[data-view-jump].quick-stat')){e.preventDefault();return}handleClick(e)});document.addEventListener('keydown',e=>{if((e.key==='Enter'||e.key===' ')&&e.target?.matches?.('[data-view-jump]:not(button)')){e.preventDefault();handleClick({target:e.target})}});$('#userSelect')?.addEventListener('change',e=>{if(window.FQAuth?.realSession){toast('Use Admin Preview from Admin Control.');populateUsers();return}state.currentUser=e.target.value;state.achievementTarget=e.target.value;if(!isAdmin()&&state.view==='admin')state.view='home';render()});$('#quickType').addEventListener('change',renderQuickFields);$('#quickForm').addEventListener('submit',submitQuick);$('#rewardForm').addEventListener('submit',submitReward);
