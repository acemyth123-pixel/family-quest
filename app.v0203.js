...
async function realRequestReward(r,details){
 ...
 let direct=null;
 try{
   const q=await window.FQAuth.client.from('reward_redemptions').select('id,status,details,requested_at,cost_rp,reward_id,user_id,reward_definitions(title,template_key)').eq('id',data).maybeSingle();
   if(!q.error)direct=q.data;
 }catch(e){console.warn('Direct reward result read failed',e)}
 const refreshes=[
   loadRealRewards().catch(e=>console.warn('Rewards refresh failed',e)),
   loadRealHouseholdMembers().catch(e=>console.warn('Members refresh failed',e)),
   loadRealXpPowerups().catch(e=>console.warn('XP power-up refresh failed',e)),
   loadRealNotifications().catch(e=>console.warn('Notification refresh failed',e))
 ];
 await Promise.all(refreshes);
 try{await loadRealCurrentStats()}catch(e){console.warn('Stats refresh failed',e)}
 let latest=(state.redemptions||[]).find(x=>String(x.id)===String(data));
 if(!latest&&direct){
   latest={id:direct.id,user:memberNameById(direct.user_id)||state.currentUser,userId:direct.user_id,rewardId:direct.reward_id,title:direct.reward_definitions?.title||r.title,templateKey:direct.reward_definitions?.template_key,cost:direct.cost_rp,status:({pending:'Pending',approved:'Approved',denied:'Denied',resolved:'Resolved'})[direct.status]||direct.status,details:direct.details||{},month:String(direct.requested_at||'').slice(0,7),requestedAt:direct.requested_at,backend:true};
   state.redemptions.unshift(latest);
 }
 const result=latest?.details?.lotteryResult||direct?.details?.lotteryResult||'';
 render();
 if(result){showLotteryResult(result);toast(`🎟️ ${result}`)}
 if(r.kind==='multiplier')toast(`⚡ ${r.templateKey==='triple_xp_day'?'3×':'2×'} XP ACTIVE UNTIL MIDNIGHT`);
 return {ok:true,status:latest?.status||direct?.status||'',result};
}
...
function openReward(id){
 ...
 if(r.kind==='lottery'&&r.backend){
   ...
   realRequestReward(r,{}).then(res=>{if(!res?.ok)console.warn('Lottery request rejected',res)}).catch(err=>{console.error(err);toast('Lottery display refresh failed, but check My Redemptions before retrying.');});
   return
 }
...
async function submitReward(e){
 ...
 if(r.backend){
   ...
   const result=await realRequestReward(r,details);
   if(result?.ok){
     $('#rewardDialog').close();
     if(r.kind==='multiplier')toast(`${r.templateKey==='triple_xp_day'?'3×':'2×'} XP activated until midnight.`);
     else if(result.status==='Approved'||result.status==='approved')toast('Reward approved automatically.');
...
if(btn.dataset.view){
 ...
 if(window.FQAuth?.realSession&&['rewards','cosmetics','home','profiles'].includes(target)){
   Promise.all([target==='rewards'?loadRealRewards():Promise.resolve(),target==='cosmetics'?loadRealCosmetics():Promise.resolve(),loadRealHouseholdMembers()]).then(()=>loadRealCurrentStats()).then(()=>render()).catch(err=>{console.error(err);render()});
 }else render();
 return
}
...
$('#rewardCancelButton').addEventListener('click',()=>{$('#rewardDialog').close()});
