const CACHE='family-quest-v0221';
const CORE=['./','./index.html','./styles.v0221.css','./app.v0221.js','./auth.v0221.js','./manifest.webmanifest','./icon-192.png','./icon-512.png'];
...
self.addEventListener('push',event=>{
 ...
 const options={body:data.body||'You have a Family Quest update.',icon:'./icon-192.png',badge:'./icon-192.png',tag:data.tag||'family-quest',renotify:false,data:{url:data.url||'/family-quest/'}};
 event.waitUntil(self.registration.showNotification(title,options));
});
self.addEventListener('notificationclick',event=>{
 event.notification.close();const target=new URL(event.notification.data?.url||'/family-quest/',self.location.origin).href;
