...
async function loadRealCosmetics(){
 ...
 state.cosmeticCatalog.forEach(d=>{const old=SPRITES.find(s=>s.id===d.id),mapped={id:d.id,name:d.name,icon:d.icon||'✨',default:d.acquisition_method==='default',achievementId:d.achievement_id||null,requirement:d.unlock_requirement||'',method:d.acquisition_method,price:d.price_rp,rarity:d.rarity,secret:!!d.secret,shopActive:!!d.shop_active,assetPath:d.asset_path||null,type:d.cosmetic_type||'avatar'};if(old)Object.assign(old,mapped);else SPRITES.push(mapped)});
