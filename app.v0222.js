...
function celebrate(title,desc){$('#achievementTitle').textContent=title;$('#achievementDesc').textContent=desc;$('#achievementPopup').classList.add('show');fireEquippedConfetti();setTimeout(()=>$('#achievementPopup').classList.remove('show'),3300)}
let lotteryPopupTimer=null;
function showLotteryResult(result){
 ...
 if(!popup||!icon||!small)return;
 if(lotteryPopupTimer){clearTimeout(lotteryPopupTimer);lotteryPopupTimer=null}
 document.querySelectorAll('.lottery-confetti').forEach(x=>x.remove());
 ...
 fireEquippedConfetti('lottery-confetti');
 lotteryPopupTimer=setTimeout(()=>{popup.classList.remove('show','lottery-pop');lotteryPopupTimer=null},3000);
}
function stat(label,value,sub,jump=''){return `<div class="card stat${jump?' quick-stat':''}" ${jump?`data-view-jump="${jump}" role="button" tabindex="0"`:''}><div class="label">${label}</div><div class="value">${value}</div><div class="sub">${sub}</div></div>`}
...
function updateShell(){const u=currentUser();$('#sidebarAvatar').innerHTML=spriteVisual(userSprite(u));$('#sidebarName').textContent=u.name;$('#sidebarRole').textContent=u.role;$('#adminNav').style.display=isAdmin()?'block':'none';const overdueCount=(state.overdueChores||[]).length;const adminBadge=$('#adminOverdueBadge');if(adminBadge){adminBadge.textContent=overdueCount?String(overdueCount):'';adminBadge.classList.toggle('show',!!overdueCount)}const unread=state.notifications.filter(n=>n.user===u.name&&!n.read).length;$('#navNotifBadge').textContent=unread?String(unread):'';$('#navNotifBadge').classList.toggle('show',!!unread);document.querySelectorAll('#nav button').forEach(b=>b.classList.toggle('active',b.dataset.view===state.view))}
...
async function loadRealCurrentStats(){
 if(!(window.FQAuth?.realSession&&window.FQAuth?.profile?.household_id))return;
 const p=window.FQAuth.profile;
 const {data,error}=await window.FQAuth.client.from('season_stats').select('season_xp,reward_points,current_streak,best_streak').eq('household_id',p.household_id).eq('user_id',p.user_id).eq('season_year',state.seasonYear).maybeSingle();
 if(error)throw error;
 const {data:life,error:lifeError}=await window.FQAuth.client.from('lifetime_stats').select('lifetime_xp').eq('user_id',p.user_id).maybeSingle();
 if(lifeError)throw lifeError;
 const u=state.users.find(x=>String(x.id)===String(p.user_id));
 if(u&&data){u.xp=data.season_xp||0;u.rp=data.reward_points||0;u.streak=data.current_streak||0;u.bestStreak=data.best_streak||0;u.lifetime=life?.lifetime_xp||u.lifetime||0}
}
...
async function realRequestReward(r,details){
 let response;
 try{response=await window.FQAuth.client.rpc('request_reward_v2',{p_reward_id:r.id,p_details:details||{}})}
 catch(err){console.error('Reward RPC transport error',err);toast('Reward request could not reach the server.');return {ok:false,error:String(err)}}
 const {data,error}=response||{};
 if(error){toast(error.message);return {ok:false,error:error.message}}
 const resultData=data||{},status=String(resultData.status||''),detailsOut=resultData.details||{},lotteryResult=detailsOut.lotteryResult||'';
 // Once the RPC succeeded, the reward is successful. Everything below is display reconciliation only.
 try{
   const me=currentUser();
   if(me){
     me.rp=Number(resultData.rewardPoints??me.rp??0);
     me.xp=Number(resultData.seasonXp??me.xp??0);
     me.streak=Number(resultData.currentStreak??me.streak??0);
     me.bestStreak=Number(resultData.bestStreak??me.bestStreak??0);
   }
   const redemptionId=resultData.redemptionId;
   if(redemptionId&&!state.redemptions.some(x=>String(x.id)===String(redemptionId))){
     state.redemptions.unshift({id:redemptionId,user:state.currentUser,userId:realProfile()?.user_id||me?.id,rewardId:r.id,title:r.title,templateKey:r.templateKey,cost:r.cost,status:({pending:'Pending',approved:'Approved',denied:'Denied',resolved:'Resolved'})[status]||status,details:detailsOut,month:monthKey(),requestedAt:new Date().toISOString(),backend:true});
   }
   render();
 }catch(displayErr){console.error('Reward succeeded; display reconciliation error',displayErr)}
 if(lotteryResult){try{showLotteryResult(lotteryResult);toast(`🎟️ ${lotteryResult}`)}catch(e){console.error('Lottery result display error',e)}}
 if(r.kind==='multiplier')toast(`⚡ ${r.templateKey==='triple_xp_day'?'3×':'2×'} XP ACTIVE UNTIL MIDNIGHT`);
 Promise.allSettled([loadRealRewards(),loadRealNotifications(),loadRealXpPowerups()]).catch(()=>{});
 return {ok:true,status,result:lotteryResult};
}
...
function renderRewards(){const u=currentUser();setHeader('REWARD SHOP',`Spend Reward Points · ${u.rp} RP`);view.innerHTML=`<div class="grid cards-3">${state.rewards.filter(r=>r.active).map(r=>{const monthly=r.kind!=='lottery'?rewardMonthlyStatus(r.id,u.name):null,powerLock=xpPowerupLockFor(r),disabled=!!powerLock||!!monthly||u.rp<r.cost;const label=powerLock?'Locked':monthly==='Pending'?'Pending Approval':monthly?'Used This Month':(r.kind==='multiplier'?'Activate':'Request');return `<div class="card${powerLock?' reward-locked':''}"><div class="kicker">REWARD</div><h3>${esc(r.title)}</h3><p class="muted">${esc(r.desc)}</p>${powerLock?`<div class="reward-lock-note">🔒 ${esc(powerLock)}</div>`:''}${monthly==='Pending'?'<div class="reward-lock-note">⏳ Waiting for Admin approval</div>':''}<div class="row" style="margin-top:18px"><span class="reward-price">💎 ${r.cost} RP</span><button class="primary" data-action="reward-open" data-id="${r.id}" ${disabled?'disabled':''}>${label}</button></div></div>`}).join('')}</div><div class="card" style="margin-top:16px"><h3>My Redemptions</h3><div class="simple-list">${state.redemptions.filter(r=>r.user===u.name).slice(0,10).map(r=>`<div class="simple-item row"><div><h4>${esc(r.title)}</h4><p>${r.cost} RP${r.details?.meal?' · '+esc(r.details.meal):''}${r.details?.choice?' · '+esc(r.details.choice):''}${r.details?.date?' · '+r.details.date:''}${r.details?.target?' · to '+r.details.target:''}${r.details?.lotteryResult?' · '+esc(r.details.lotteryResult):''}</p></div><span class="chip ${r.status==='Pending'?'warn':(r.status==='Approved'||r.status==='Resolved')?'good':'bad'}">${r.status}</span></div>`).join('')||'<div class="empty">No reward requests.</div>'}</div></div>`}
...
async function loadRealGroceryQuest(){
 if(!(window.FQAuth?.realSession&&window.FQAuth?.profile?.household_id))return;
...
function renderGroceries(){setHeader('SHARED LIST','Groceries');const q=state.groceryQuest||{},members=q.members||[],ready=Number(q.readyCount||0),total=Number(q.totalCount||state.users.length),due=q.dueAt?fmt(q.dueAt):'Not scheduled',days=['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'];view.innerHTML=`<div class="card grocery-quest-card"><div class="section-title"><div><div class="kicker">WEEKLY HOUSEHOLD QUEST</div><h3>🛒 Grocery List Quest</h3><p class="muted">Due ${due} · ${ready}/${total} lists ready</p></div>${!q.myReady?`<button class="primary" data-action="grocery-ready">My Grocery List Is Ready · +${q.listXp??10} XP</button>`:'<span class="chip good">✓ Your List Is Ready</span>'}</div><div class="grocery-ready-grid">${members.map(m=>`<div class="simple-item"><strong>${esc(m.name)}</strong><span class="chip ${m.readyAt?'good':''}">${m.readyAt?'✓ Ready':'Waiting'}</span></div>`).join('')}</div>${isAdmin()?`<div class="action-row grocery-admin-actions"><button class="good" data-action="grocery-run-complete">Complete Grocery Run · +${q.runXp??200} XP</button><span class="muted">Admin completes the actual shopping here.</span></div>`:''}</div><div class="card" style="margin-top:16px"><div class="section-title"><h3>Current List</h3><button class="primary" data-action="quick-open" data-type="grocery">+ Add Item</button></div><div class="notice">Everyone can add what they need. Everyone can see the list and status. Only Admins can change item status.</div><div class="simple-list" style="margin-top:12px">${state.groceries.map(g=>`<div class="simple-item"><div class="row"><div><h4>${esc(g.item)}</h4><p>Requested by ${esc(g.by)}${g.notes?' · '+esc(g.notes):''}</p></div><span class="chip">${esc(g.status)}</span></div>${isAdmin()?`<div class="action-row" style="margin-top:10px">${['Requested','In Cart','Purchased','Delayed','Denied'].map(x=>`<button class="${x==='Purchased'?'good':x==='Denied'?'bad':x==='Delayed'?'warn':'ghost'}" data-action="grocery-status" data-id="${g.id}" data-status="${x}" ${g.status===x?'disabled':''}>${x}</button>`).join('')}</div>`:''}</div>`).join('')||'<div class="empty">Nothing on the grocery list.</div>'}</div></div>`}
...
async function loadRealFamilyRequests(){
 if(!(window.FQAuth?.realSession&&window.FQAuth?.profile?.household_id))return;
 const {data,error}=await window.FQAuth.client.from('family_requests').select('id,title,notes,requested_by,requested_for,start_at,end_at,status,responded_by,responded_at,created_at').eq('household_id',window.FQAuth.profile.household_id).order('created_at',{ascending:false});
 if(error){console.error('Family requests load failed',error);return}
 state.requests=(data||[]).map(r=>({id:r.id,title:r.title,notes:r.notes||'',by:memberNameById(r.requested_by)||'Family Member',requestedBy:r.requested_by,requestedFor:r.requested_for,requestedForName:r.requested_for?memberNameById(r.requested_for):'',start:r.start_at,end:r.end_at,status:r.status,owner:r.responded_by?memberNameById(r.responded_by):'',respondedBy:r.responded_by,backend:true}));
}
async function realCreateFamilyRequest(title,start,end,notes,requestedFor=null){const {error}=await window.FQAuth.client.rpc('create_family_request',{p_title:title,p_start_at:new Date(start).toISOString(),p_end_at:new Date(end).toISOString(),p_notes:notes||null,p_requested_for:requestedFor||null});if(error){toast(error.message);return false}await Promise.all([loadRealFamilyRequests(),loadRealNotifications()]);toast(`${title} requested.`);render();return true}
...
function renderRequests(){setHeader('FAMILY REQUESTS','Ask the House');view.innerHTML=`<div class="card"><div class="section-title"><h3>Requests</h3><button class="primary" data-action="quick-open" data-type="request">+ New Request</button></div><div class="simple-list">${state.requests.map(r=>`<div class="simple-item"><div class="row"><div><h4>${esc(r.title)}</h4><p>${fmt(r.start)} → ${fmt(r.end)} · Requested by ${r.by}${r.requestedForName?' · 🎯 For '+esc(r.requestedForName):''}${r.owner?' · Accepted by '+r.owner:''}</p></div><span class="chip ${r.status==='Denied'?'bad':r.status==='Accepted'?'good':''}">${r.status}</span></div>${r.status==='Open'&&r.requestedBy!==window.FQAuth?.profile?.user_id?`<div class="action-row" style="margin-top:10px"><button class="good" data-action="request-accept" data-id="${r.id}">I'll Do It</button><button class="bad" data-action="request-deny" data-id="${r.id}">Deny</button></div>`:r.status==='Open'?'<div class="muted" style="margin-top:10px">Waiting for another family member.</div>':''}</div>`).join('')}</div></div>`}

async function loadRealCosmetics(){
 if(!(window.FQAuth?.realSession&&window.FQAuth?.profile?.household_id))return;
 const c=window.FQAuth.client;
 const [defs,unlocks]=await Promise.all([c.from('cosmetic_definitions').select('id,name,icon,acquisition_method,price_rp,unlock_requirement,achievement_id,rarity,secret,active,shop_active,sort_order,asset_path,cosmetic_type').eq('active',true).order('sort_order'),c.from('cosmetic_unlocks').select('user_id,sprite_id,source_achievement_id,source_type,purchase_price_rp')]);
 if(defs.error)throw defs.error;if(unlocks.error)throw unlocks.error;
 state.cosmeticCatalog=(defs.data||[]).map(d=>({...d,type:d.cosmetic_type||'avatar',assetPath:d.asset_path||null}));
 state.cosmeticCatalog.forEach(d=>{const old=SPRITES.find(s=>s.id===d.id),mapped={id:d.id,name:d.name,icon:d.icon||'✨',default:d.acquisition_method==='default',achievementId:d.achievement_id||null,requirement:d.unlock_requirement||'',method:d.acquisition_method,price:d.price_rp,rarity:d.rarity,secret:!!d.secret,shopActive:!!d.shop_active,assetPath:d.assetPath||null,type:d.type||'avatar'};if(old)Object.assign(old,mapped);else SPRITES.push(mapped)});
...
function confettiSpec(id){
 const specs={
  'leaf-confetti':['🍂','🍁','🍃'],'snowfall-confetti':['❄️','❅','✦'],'harvest-confetti':['🍎','🎃','🍂'],'fireworks-confetti':['✦','★','✹'],'coin-confetti':['🪙','◆','✦'],'grass-confetti':['🌱','🍃','▪'],'sunshine-confetti':['🌈','☀️','✦'],'bubble-confetti':['🫧','○','◌'],'ice-confetti':['🧊','❄️','✦']
 }; return specs[id]||null;
}
function fireEquippedConfetti(extraClass=''){
 const id=currentUser()?.confettiId||'classic-confetti',spec=confettiSpec(id),count=spec?48:45;
...
function renderCosmetics(){
 ...
 const card=x=>{const has=x.acquisition_method==='default'||owned.has(x.id),can=u.rp>=Number(x.price_rp||0),ach=state.achievements.find(a=>String(a.id)===String(x.achievement_id)),earnedAch=ach&&(ach.earnedBy||[]).includes(u.name),equipped=has&&cosmeticEquipped(u,x);return `<div class="card cosmetic-card rarity-${esc(x.rarity||'common')} ${has?'cosmetic-owned':''}"><div class="cosmetic-preview">${has||x.acquisition_method==='rp_shop'?cosmeticPreview(x):'<span class="locked-sprite">🔒</span>'}</div><div class="kicker">${esc(cosmeticRarityLabel(x.rarity))} · ${esc(cosmeticTypeLabel(x.type))}</div><h3>${esc(has||!x.secret?x.name:'Secret Cosmetic')}</h3><p class="muted">${esc(has||!x.secret?(x.unlock_requirement||'Permanent cosmetic unlock.'):'Secret achievement')}</p><div class="row" style="margin-top:18px">${x.acquisition_method==='rp_shop'?`<span class="reward-price">💎 ${Number(x.price_rp||0)} RP</span>${has?`<button class="${equipped?'good':'ghost'}" data-action="cosmetic-equip" data-cosmetic="${esc(x.id)}">${equipped?'✓ Equipped':'Equip'}</button>`:`<button class="primary" data-action="cosmetic-buy" data-cosmetic="${esc(x.id)}" ${can?'':'disabled'}>${can?'Buy':'Need RP'}</button>`}`:`<span class="achievement-cosmetic-status">${has?'✓ Unlocked':earnedAch?'🏆 Ready to Claim':'🔒 Achievement'}</span>${has?`<button class="${equipped?'good':'ghost'}" data-action="cosmetic-equip" data-cosmetic="${esc(x.id)}">${equipped?'✓ Equipped':'Equip'}</button>`:''}`}</div></div>`};
 const section=(type,title)=>{const items=earned.filter(x=>(x.type||'avatar')===type);return `<h3 class="section-title cosmetic-achievement-title">${title}</h3><div class="grid cards-3 cosmetic-shop-grid">${items.map(card).join('')||'<div class="empty">No achievement cosmetics in this category yet.</div>'}</div>`};
 view.innerHTML=`<div class="card cosmetic-shop-intro"><div><div class="kicker">COSMETIC STORE</div><h3>Build your Family Quest look</h3><p class="muted">Mix an avatar, profile frame, profile background and celebration confetti. Achievement cosmetics are permanent once claimed.</p></div><button class="ghost" data-action="profile-open">Customize Profile</button></div>${shop.length?`<h3 class="section-title">RP Shop</h3><div class="grid cards-3 cosmetic-shop-grid">${shop.map(card).join('')}</div>`:''}${section('avatar','Achievement Avatars')}${section('confetti','Achievement Confetti')}${section('frame','Achievement Frames')}${section('background','Achievement Backgrounds')}`;
}
async function loadRealAchievements(){
 try{if(window.FQAuth?.realSession){await window.FQAuth.client.rpc('refresh_behavioral_achievements');await window.FQAuth.client.rpc('refresh_sheet_defined_achievements')}}catch(e){console.warn('Achievement refresh skipped',e)}
 if(!(window.FQAuth?.realSession&&window.FQAuth?.profile?.household_id))return;
...
window.FQLoaders.groceries=async()=>{await loadRealGroceries();await loadRealGroceryQuest()};
...
function pushControlsMarkup(){
 ...
 if(!state.pushSupported)return `<div class="phone-push-box"><strong>📱 Phone Notifications</strong><p class="muted">This browser does not currently support Family Quest phone notifications. On iPhone/iPad, add Family Quest to the Home Screen and open it there first.</p></div>`;
 const enabled=!!state.pushEnabled,denied=state.pushPermission==='denied';
 return `<div class="phone-push-box"><strong>📱 Phone Notifications</strong><p class="muted">Overdue chores, approval results, Family Requests, and today's calendar items.</p><div class="action-row"><button class="${enabled?'good':'primary'}" data-action="push-toggle" ${denied?'disabled':''}>${enabled?'Notifications Enabled':'Enable Phone Notifications'}</button>${enabled?'<button class="ghost" data-action="push-test">Send Test Notification</button>':''}</div>${denied?'<p class="muted">Notifications are blocked in your browser settings. Allow notifications for Family Quest, then reopen the app.</p>':''}</div>`;
}
function renderPushControlsOnly(){const host=document.querySelector('#phonePushControls');if(host)host.innerHTML=pushControlsMarkup()}
async function enablePhonePush(){
 if(!pushSupported()){toast('Phone notifications are not supported in this browser.');return}
 try{
...
async function disablePhonePush(){
 try{const reg=await getPushRegistration(),sub=reg?await reg.pushManager.getSubscription():null;if(sub){await window.FQAuth.client.rpc('remove_push_subscription',{p_endpoint:sub.endpoint});await sub.unsubscribe()}state.pushEnabled=false;toast('Phone notifications disabled.');renderPushControlsOnly()}catch(e){toast(e?.message||'Could not disable notifications.')}
}
async function sendTestPhonePush(){const {error}=await window.FQAuth.client.rpc('create_test_push_notification');if(error){toast(error.message);return}toast('Test notification sent. Give it a few seconds.')}
 ...
 <div class="grid cards-3" style="margin-top:16px">
   ${state.users.map(u=>{
      ...
      return `<div class="card">
        <div class="profile-card">${profileShowcase(u)}<div><strong>${u.name}</strong><span>${u.role}</span><span class="sprite-name">${esc(s.name)}</span></div></div>
        <p class="muted">${esc(u.title)}<br>Lifetime XP: ${u.lifetime.toLocaleString()}<br>Best streak: ${u.bestStreak} days<br>Reward Points: ${u.rp}<br>Cosmetics Unlocked: ${(u.cosmeticUnlocks||[]).length + 13}</p>
        ${u.name===state.currentUser?'<div class="action-row"><button class="ghost" data-action="profile-open">Customize Profile</button><button class="ghost" type="button" data-auth-action="signout">↪ Sign Out</button></div><div id="phonePushControls">'+pushControlsMarkup()+'</div>':''}
      </div>`;
   }).join('')}
 </div>`;
 if(window.FQAuth?.realSession)setTimeout(refreshPushState,0);
}
function renderNotifications(){setHeader('INBOX','Notifications');const ns=state.notifications.filter(n=>n.user===state.currentUser),unread=ns.filter(n=>!n.read).length;view.innerHTML=`<div class="card"><div class="section-title"><h3>Inbox</h3><div class="action-row">${unread?'<button class="ghost" data-action="notif-read-all">Mark all read</button>':''}${ns.length?'<button class="ghost" data-action="notif-clear">Clear Inbox</button>':''}</div><div class="simple-list">${ns.map(n=>`<div class="simple-item row ${n.read?'':'notification-unread'}"><div><h4>${n.read?'Read':'● New'}</h4><p>${esc(n.text)}</p></div>${n.read?'':`<button class="ghost" data-action="notif-read" data-id="${n.id}">Mark Read</button>`}</div>`).join('')||'<div class="empty">No notifications.</div>'}</div></div>`}
...
function overdueAge(due){const ms=Date.now()-new Date(due).getTime();if(ms<0)return 'Not overdue';const mins=Math.floor(ms/60000);if(mins<60)return `${mins} min overdue`;const hrs=Math.floor(mins/60);if(hrs<24)return `${hrs} hr${hrs===1?'':'s'} overdue`;const days=Math.floor(hrs/24),rem=hrs%24;return `${days} day${days===1?'':'s'}${rem?` ${rem} hr`:''} overdue`}
async function loadAdminOverdueChores(){
 if(!(window.FQAuth?.realSession&&isAdmin())){state.overdueChores=[];return []}
 const {data,error}=await window.FQAuth.client.rpc('get_admin_overdue_chores');if(error)throw error;
 state.overdueChores=data||[];updateShell();return state.overdueChores;
}
async function sendOverdueReminder(instanceId){
 const btn=document.querySelector(`[data-action="overdue-remind"][data-instance="${CSS.escape(String(instanceId))}"]`);if(btn){btn.disabled=true;btn.textContent='Sending…'}
 const {error}=await window.FQAuth.client.rpc('send_overdue_chore_reminder',{p_instance_id:instanceId});
 if(error){toast(error.message);if(btn){btn.disabled=false;btn.textContent='Send Reminder'}return}
 toast('Overdue reminder sent.');await Promise.allSettled([loadAdminOverdueChores(),loadRealNotifications()]);render();
}
function renderOverduePanel(){
 if(!isAdmin())return '';
 const list=state.overdueChores||[];
 return `<div class="card admin-section overdue-panel"><div class="section-title"><h3>⏰ Overdue Chores</h3><span class="chip ${list.length?'bad':''}">${list.length}</span></div><p class="muted">Open assigned chores past their due time. Reminders are limited to once every 6 hours per chore.</p><div class="simple-list">${list.map(x=>`<div class="simple-item row"><div><h4>${esc(x.title)}</h4><p>${esc(x.assigned_name||'Family Member')} · ${esc(overdueAge(x.due_at))} · Due ${fmt(x.due_at)}</p>${x.last_reminder_at?`<p class="muted">Last reminder: ${fmt(x.last_reminder_at)}</p>`:''}</div><div class="action-row"><button class="${x.can_remind?'warn':'ghost'}" data-action="overdue-remind" data-instance="${x.instance_id}" ${x.can_remind?'':'disabled'}>${x.can_remind?'Send Reminder':'Reminder Sent'}</button></div></div>`).join('')||'<div class="empty">No overdue chores. Nice.</div>'}</div></div>`;
}
function renderAdmin(){if(!isAdmin()){state.view='home';render();return}setHeader('ADMIN CONTROL','Manage Household');const pc=realChoresEnabled()?[]:state.chores.filter(c=>c.status==='Pending'),pr=state.redemptions.filter(r=>r.status==='Pending'&&!r.backend);
 ...
 const gq=state.groceryQuest||{},gdays=['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'];
 const grocerySettingsPanel=`<div class="card admin-section"><div class="section-title"><h3>🛒 Grocery Quest Settings</h3><span class="muted">Weekly household prep + grocery run</span></div><div class="form-grid"><label>Weekly Due Day<select id="groceryDueDay">${gdays.map((d,i)=>`<option value="${i}" ${Number(gq.dueWeekday)===i?'selected':''}>${d}</option>`).join('')}</select></label><label>Grocery List Quest XP<input id="groceryListXp" type="number" min="0" value="${gq.listXp??10}"></label><label>Grocery Run XP<input id="groceryRunXp" type="number" min="0" value="${gq.runXp??200}"></label><div class="action-row"><button class="primary" data-action="grocery-settings-save">Save Grocery Settings</button></div></div></div>`;
const seasonPanel=`
...
function renderOverduePanelIntoAdmin(){
 if(state.view!=='admin')return;const old=document.querySelector('.overdue-panel');if(!old)return;
 const wrap=document.createElement('div');wrap.innerHTML=renderOverduePanel();const fresh=wrap.firstElementChild;if(fresh)old.replaceWith(fresh);updateShell();
}
function render(){
 ...
 if(state.view==='admin'&&window.FQAuth?.realSession){setTimeout(()=>window.FQAuth.renderMembershipAdmin(),0);setTimeout(renderRealApprovalQueue,0);setTimeout(renderRealRewardApprovalQueue,0);setTimeout(()=>loadAdminOverdueChores().then(()=>{if(state.view==='admin')renderOverduePanelIntoAdmin()}).catch(e=>toast(e.message||String(e))),0)}
}
...
function renderQuickFields(){const t=$('#quickType').value;$('#quickTitle').readOnly=false;$('#quickNotes').readOnly=false;if(t==='calendar'||t==='request')$('#quickDynamic').innerHTML=`<div class="form-grid"><label>Start<input type="datetime-local" id="quickStart" required></label><label>End<input type="datetime-local" id="quickEnd" required></label>${t==='calendar'?'<label class="full">For<input id="quickFor" placeholder="Household"></label>':`<label class="full">Requested For (optional)<select id="quickRequestedFor"><option value="">Anyone / Household</option>${state.users.filter(u=>u.id!==window.FQAuth?.profile?.user_id).map(u=>`<option value="${u.id}">${esc(u.name)}</option>`).join('')}</select></label>`}</div>`;else $('#quickDynamic').innerHTML=''}
...
function openReward(id){
 ...
 if(u.rp<r.cost){toast('Not enough RP for this reward.');return}
 if(r.kind!=='lottery'){
   const monthly=rewardMonthlyStatus(id,u.name);
   if(monthly){toast(monthly==='Pending'?'This reward is already pending Admin approval.':'This reward was already used this month.');return}
 }
 if(r.kind==='lottery'&&r.backend){
   const now=Date.now();
   if(rewardRequestBusy||now<lotteryBusyUntil){toast('🎟️ Lottery is still drawing…');return}
   rewardRequestBusy=true;lotteryBusyUntil=now+4000;
   toast('🎟️ Drawing your RP Lottery ticket…');
   realRequestReward(r,{}).finally(()=>{setTimeout(()=>{rewardRequestBusy=false},Math.max(0,lotteryBusyUntil-Date.now()))});
   return
 }
...
async function submitReward(e){
 ...
 if(r.backend){
   ...
   if(rewardRequestBusy){toast('Reward request is already being submitted…');return}
   rewardRequestBusy=true;
   const submitBtn=$('#rewardSubmitButton');
   if(submitBtn){submitBtn.disabled=true;submitBtn.textContent='Submitting…'}
   // Close immediately after validation so mobile browsers cannot leave a stale modal
   // on screen while the async request/render cycle runs.
   ...
   try{
     ...
     console.warn('Reward dialog close fallback',closeErr);
     dlg?.removeAttribute('open');
   }
   toast('Submitting reward…');
   let result;
   try{result=await realRequestReward(r,details)}finally{
     rewardRequestBusy=false;
     if(submitBtn){submitBtn.disabled=false;submitBtn.textContent='Submit Request'}
   }
   if(result?.ok){
     if(r.kind==='multiplier')toast(`${r.templateKey==='triple_xp_day'?'3×':'2×'} XP activated until midnight.`);
     else if(String(result.status).toLowerCase()==='approved')toast('Reward approved automatically.');
     else if(r.kind!=='lottery')toast('Reward sent for approval.');
   }else{
     toast('Reward was not submitted.');
   }
   return;
 }
...
function handleClick(e){const btn=e.target.closest('button,[data-action],[data-view-jump]');if(!btn)return;const action=btn.dataset.action;
...
if(btn.dataset.view){
 const target=btn.dataset.view;state.view=target;
 if(window.FQAuth?.realSession&&['rewards','cosmetics','home','profiles'].includes(target)){
   loadRealHouseholdMembers().then(()=>loadRealCurrentStats()).then(()=>Promise.allSettled([target==='rewards'?loadRealRewards():Promise.resolve(),target==='cosmetics'?loadRealCosmetics():Promise.resolve()])).then(()=>render()).catch(err=>{console.error(err);render()});
 }else if(window.FQAuth?.realSession&&target==='admin'){loadAdminOverdueChores().then(()=>render()).catch(err=>{console.error(err);render()});}
 else render();
 return
}
if(btn.dataset.viewJump){state.view=btn.dataset.viewJump;render();return}if(action==='cosmetic-buy'){purchaseCosmetic(btn.dataset.cosmetic);return}
 if(action==='cosmetic-equip'){equipCosmetic(btn.dataset.cosmetic).then(ok=>{if(ok)render()});return}if(action==='achievement-claim'){realClaimAchievement(btn.dataset.id);return}if(action==='reward-save-real'){realSaveRewardEdits(btn.dataset.redemption);return}if(action==='reward-approve-real'){realReviewReward(btn.dataset.redemption,'approve');return}if(action==='reward-deny-real'){realReviewReward(btn.dataset.redemption,'deny');return}if(!action)return;if(action==='dialog-close'){$('#'+btn.dataset.dialog).close();return}if(action==='quick-open'){openQuick(btn.dataset.type||'grocery');return}if(action==='test-achievement'){celebrate('I Was Getting To It','Completed a chore suspiciously close to the deadline.');return}if(action==='chore-detail'){openChoreDetail(btn.dataset.id);return}if(action==='chore-claim'){e.stopPropagation();const c=state.chores.find(x=>String(x.id)===String(btn.dataset.id));if(c?.backend){realClaimChore(c);return}claimChore(Number(btn.dataset.id));return}if(action==='chore-complete'){e.stopPropagation();const c=state.chores.find(x=>String(x.id)===String(btn.dataset.id));if(c?.backend){realCompleteChore(c);return}completeChore(Number(btn.dataset.id));return}if(action==='chore-approve'&&btn.dataset.completion){realReviewCompletion(btn.dataset.completion,'approve');return}if(action==='chore-sendback'&&btn.dataset.completion){realReviewCompletion(btn.dataset.completion,'send_back');return}if(action==='chore-deny'&&btn.dataset.completion){realReviewCompletion(btn.dataset.completion,'deny');return}if(action==='chore-approve'){approveChore(Number(btn.dataset.id));return}if(action==='chore-sendback'){sendBackChore(Number(btn.dataset.id));return}if(action==='chore-deny'){denyChore(Number(btn.dataset.id));return}if(action==='grocery-ready'){markGroceryListReady();return}if(action==='grocery-run-complete'){completeGroceryRun();return}if(action==='grocery-settings-save'){saveGroceryQuestSettings();return}if(action==='grocery-status'){const g=state.groceries.find(x=>String(x.id)===String(btn.dataset.id));if(g?.backend){realSetGroceryStatus(btn.dataset.id,btn.dataset.status);return}setGroceryStatus(Number(btn.dataset.id),btn.dataset.status);return}if(action==='request-accept'){const r=state.requests.find(x=>String(x.id)===String(btn.dataset.id));if(r?.backend){realRespondFamilyRequest(btn.dataset.id,'accept');return}acceptRequest(Number(btn.dataset.id));return}if(action==='request-deny'){const r=state.requests.find(x=>String(x.id)===String(btn.dataset.id));if(r?.backend){realRespondFamilyRequest(btn.dataset.id,'deny');return}denyRequest(Number(btn.dataset.id));return}if(action==='event-edit'){e.stopPropagation();openQuick('calendar',btn.dataset.id);return}if(action==='event-remove'){e.stopPropagation();removeEvent(btn.dataset.id);return}if(action==='day-zoom'){openDayZoom(btn.dataset.date);return}if(action==='day-full'){state.calendarCursor=btn.dataset.date;state.calendarMode='day';$('#dayDialog').close();render();return}if(action==='calendar-mode'){state.calendarMode=btn.dataset.mode;render();return}if(action==='calendar-move'){moveCalendar(Number(btn.dataset.dir));return}if(action==='calendar-today'){calendarToday();return}if(action==='reward-open'){return}if(action==='reward-approve'){approveReward(Number(btn.dataset.id));return}if(action==='reward-deny'){denyReward(Number(btn.dataset.id));return}if(action==='achievement-target'){state.achievementTarget=btn.dataset.user;render();return}if(action==='notif-clear'){if(window.FQAuth?.realSession){realClearNotifications();return}if(confirm('Clear all notifications from your inbox?')){state.notifications=state.notifications.filter(n=>n.user!==state.currentUser);render()}return}if(action==='notif-read-all'){if(window.FQAuth?.realSession){realMarkAllNotificationsRead();return}state.notifications.filter(n=>n.user===state.currentUser).forEach(n=>n.read=true);render();return}if(action==='notif-read'){const n=state.notifications.find(x=>String(x.id)===String(btn.dataset.id));if(n?.backend){realMarkNotificationRead(btn.dataset.id);return}if(n)n.read=true;render();return}if(action==='push-toggle'){if(state.pushEnabled)disablePhonePush();else enablePhonePush();return}if(action==='push-test'){sendTestPhonePush();return}if(action==='overdue-remind'){sendOverdueReminder(btn.dataset.instance);return}if(action==='achievement-visibility'){setAchievementVisibility(btn.dataset.id,btn.dataset.secret==='true');return}if(action==='admin-create'){openAdminEditor(btn.dataset.type);return}if(action==='admin-toggle'){if(btn.dataset.type==='chore'&&realChoresEnabled()){realToggleChore(btn.dataset.id);return}toggleActive(btn.dataset.type,btn.dataset.type==='reward'?btn.dataset.id:Number(btn.dataset.id));return}if(action==='admin-edit'){openAdminEditor(btn.dataset.type,(btn.dataset.type==='chore'&&realChoresEnabled())||btn.dataset.type==='reward'?btn.dataset.id:Number(btn.dataset.id));return}if(action==='admin-remove'){if(btn.dataset.type==='chore'&&realChoresEnabled()){toast('Deactivate real chores instead of deleting them so history is preserved.');return}removeAdminItem(btn.dataset.type,Number(btn.dataset.id));return}if(action==='profile-cosmetic-select'){const el=$('#'+btn.dataset.input);if(el){el.value=btn.dataset.cosmetic;renderCosmeticPicker()}return}if(action==='sprite-select'){const s=spriteById(btn.dataset.sprite);if(!spriteUnlocked(currentUser(),s))return;$('#profileAvatarId').value=s.id;renderSpritePicker();return}if(action==='profile-open'){openProfile();return}}

document.addEventListener('click',e=>{
 const btn=e.target.closest?.('[data-action="reward-open"]');
 if(!btn)return;
 e.preventDefault();e.stopPropagation();
 openReward(btn.dataset.id);
},true);
let quickNavGesture=null,quickNavHandledAt=0;
document.addEventListener('pointerdown',e=>{const card=e.target.closest?.('[data-view-jump].quick-stat');if(!card||e.pointerType==='mouse')return;quickNavGesture={pointerId:e.pointerId,card,startX:e.clientX,startY:e.clientY,moved:false,startedAt:Date.now()}},{passive:true});
document.addEventListener('pointermove',e=>{if(!quickNavGesture||quickNavGesture.pointerId!==e.pointerId)return;const dx=e.clientX-quickNavGesture.startX,dy=e.clientY-quickNavGesture.startY;if(Math.hypot(dx,dy)>10)quickNavGesture.moved=true},{passive:true});
document.addEventListener('pointercancel',e=>{if(quickNavGesture?.pointerId===e.pointerId)quickNavGesture=null},{passive:true});
document.addEventListener('pointerup',e=>{if(!quickNavGesture||quickNavGesture.pointerId!==e.pointerId)return;const g=quickNavGesture;quickNavGesture=null;if(g.moved||Date.now()-g.startedAt>800)return;const released=e.target.closest?.('[data-view-jump].quick-stat');if(released!==g.card)return;e.preventDefault();quickNavHandledAt=Date.now();state.view=g.card.dataset.viewJump;render()},{passive:false});
document.addEventListener('click',e=>{if(Date.now()-quickNavHandledAt<500&&e.target.closest?.('[data-view-jump].quick-stat')){e.preventDefault();return}handleClick(e)});document.addEventListener('keydown',e=>{if((e.key==='Enter'||e.key===' ')&&e.target?.matches?.('[data-view-jump]:not(button)')){e.preventDefault();handleClick({target:e.target})}});$('#userSelect')?.addEventListener('change',e=>{if(window.FQAuth?.realSession){toast('Use Admin Preview from Admin Control.');populateUsers();return}state.currentUser=e.target.value;state.achievementTarget=e.target.value;if(!isAdmin()&&state.view==='admin')state.view='home';render()});$('#quickType').addEventListener('change',renderQuickFields);$('#quickForm').addEventListener('submit',submitQuick);$('#rewardForm').addEventListener('submit',submitReward);
$('#rewardCancelButton').addEventListener('click',()=>{state.rewardDraft=null;$('#rewardDialog').close()});
$('#rewardSubmitButton').addEventListener('click',e=>{if(e.detail===0)return});$('#adminEditorForm').addEventListener('submit',submitAdminEditor);$('#profileForm').addEventListener('submit',submitProfile);$('#cosmeticPriceForm').addEventListener('submit',submitCosmeticPrice);$('#userForm').addEventListener('submit',submitUser);
